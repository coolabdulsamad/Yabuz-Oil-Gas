...
function NewLoanDialog({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  ...
  return (
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="p-3 sm:p-6 max-h-[92vh] w-[95vw] max-w-3xl sm:max-w-3xl overflow-x-hidden overflow-y-auto">
        <DialogHeader>
...
function LoanDetail({ id, onClose, onChanged }: { id: number; onClose: () => void; onChanged: () => void }) {
  ...
  return (
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="p-3 sm:p-6 max-h-[92vh] w-[95vw] max-w-4xl sm:max-w-4xl overflow-y-auto">
        <DialogHeader>
